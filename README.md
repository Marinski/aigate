...
| **[piston](https://github.com/engineer-man/piston)** _(optional, `PISTON=1`)_                                  | Sandboxed multi-language code execution. 50+ languages available (Python / Node / Bash / Deno / Go / Rust / TypeScript installed by default; full list at the upstream pkgs repo). nsjail-based isolation per execution — own user namespace, chroot, seccomp filter, cgroups for CPU+memory+wall-clock+process-count caps. `PISTON_DISABLE_NETWORKING=true` by default so sandboxed code has no outbound network. The `piston-pull` sidecar installs the language set from `PISTON_LANGUAGES` on first `up` (idempotent on subsequent runs). REST API at `/piston/api/v2/*` behind nginx bearer auth + an `execute_code` MCP tool any function-calling LLM can call. Container runs `privileged: true` — that's what nsjail needs to set up isolation primitives, not what isolates the code (which lives in the nsjail subprocess inside). |
...
| **[predictalot](https://github.com/psyb0t/docker-predictalot)** _(optional, `PREDICTALOT=1` and/or `PREDICTALOT_CUDA=1`)_ | Foundation time-series forecasting. REST + MCP. Direct route (not via LiteLLM). Models lazy-load on first request, auto-unload when idle. CPU and CUDA variants run side-by-side on distinct routes (`/predictalot/` and `/predictalot-cuda/`) and aliases, both sharing `.data/predictalot/models/`. CUDA needs `nvidia-container-toolkit`. Full API in the [upstream README](https://github.com/psyb0t/docker-predictalot). |
