...
- **talkies** _(TALKIES=1 / TALKIES_CUDA=1)_ — `/healthz` reachability, `/v1/models` lists the configured model_ids (CPU: 4 ASR + Kokoro; CUDA: 7 ASR + Kokoro), `/api/ps` returns the loaded list, `DELETE /api/ps/{model_id}` accepts URL-encoded slugs (404 on never-loaded model_ids), `POST /unload` returns 200, and end-to-end transcription + TTS through the LiteLLM proxy (asserts non-empty `text` field, MP3 body for `/v1/audio/speech`). The e2e transcription test is gated on the presence of `tests/.fixtures/audio.{wav,mp3,m4a,flac,ogg}` (skipped if missing — ffmpeg-decoded inside the service so any format works).
