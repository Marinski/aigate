...
class MultitaskBackend:
    def __init__(self, model_id: str, repo: str, device: str) -> None:
        ...
        self._log = logging.getLogger(f"talkies.multitask.{model_id}")
    ...
    def _transcribe_dispatch(
        ...
        if duration <= config.VAD_CHUNK_THRESHOLD_SECONDS:
            res = self._transcribe_path(
                model, audio_path, source_lang, target_lang, task, with_timestamps
            )
            res.language = source_lang
        ...
        tmpdir = tempfile.mkdtemp(prefix="talkies-canary-")
        ...
        try:
            for i, region in enumerate(chunks):
                ...
                res = self._transcribe_path(
                    model, chunk_path, source_lang, target_lang, task, with_timestamps
                )
